...
def run(path, prev_model=False):
    ...
    par_eval = neat.ParallelEvaluator(num_workers=15, eval_function=eval_genomes)
...
# A class for our worker that contains the genome and config objects that
# we can use to run our network
class Worker(object):
    # the init just assigns the passed genome and config objects to our self
    # referenced objects
    def __init__(self, genome, config):
        self.genome = genome
        self.config = config

    # The work function if the function used to determine our fitness
    def work(self):
        # Define our environment into a self referential variable called env
        self.env = retro.make('SuperMarioBros3-Nes', '1Player.World1.Level1.state')
        ...
        screen_x, screen_y, screen_z = self.env.observation_space.shape
        ...
        while not done:
            ...
            ob, rew, done, ram_val = self.env.step(nn_Output)
